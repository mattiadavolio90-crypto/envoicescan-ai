...
MIGRATION 070 - Nuova colonna piva_cedente
=============================================

**AGGIUNTA 2026-05-10:** Migration rinumerata come 070 per mantenere sequenza intera lineare.

☑ ALTER TABLE fatture ADD COLUMN IF NOT EXISTS piva_cedente TEXT DEFAULT NULL;
   ✅ migrations/070_add_piva_cedente_to_fatture.sql

**Scopo:** Salvare P.IVA del cedente (fornitore emittente) a livello di riga per:
- Utilizzo da parte di migration 072 backfill per popolare fatture_documenti.piva_fornitore
...
2. **070:** ALTER TABLE fatture ADD COLUMN piva_cedente (supporto Step 2 parser)
3. **071:** CREATE TABLE fornitori_pagamenti_config (regole fornitore)
4. **072:** Backfill fatture_documenti da fatture (legge piva_cedente da colonna aggiunta in 070)

---

CORREZIONE 072 - piva_cedente invece di piva_cessionario
=========================================================

**CORREZIONE 2026-05-10:** Migration 072 linea 85-86 corretta.
...
Migration 070 aggiunge la colonna piva_cedente a fatture; migration 072 la legge per popolare fatture_documenti.
   - Pulizia temporaneo
...
Passo 2: Applica 070
   - Esegui: psql -h db.supabase.co -d postgres -U postgres -f 070_add_piva_cedente_to_fatture.sql
   - Verifica: SELECT column_name FROM information_schema.columns WHERE table_name='fatture' AND column_name='piva_cedente';

Passo 3: Applica 071
   - Esegui: psql -h db.supabase.co -d postgres -U postgres -f 071_create_fornitori_pagamenti_config.sql
   - Verifica: SELECT COUNT(*) FROM fornitori_pagamenti_config; (deve ritornare 0, tabella esiste)

Passo 4: Applica 072
   - Esegui: psql -h db.supabase.co -d postgres -U postgres -f 072_backfill_fatture_documenti.sql
   - Verifica: 
...
PRONTO PER APPROVAZIONE: 069-072
