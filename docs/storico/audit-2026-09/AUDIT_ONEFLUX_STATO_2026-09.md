...
**Ciclo aperto il 29/08/2026, tuttora in corso. Stato aggiornato al 06/09/2026 (dopo `408a7e6`, non pushato).**

> **Copertura app: 97%** (111.202 su 114.518). Backend ed Edge Functions **100%**;
...
> difetti *di quella classe* e tace sulle altre.
...
| **4** | **I router del worker — APERTA, 2 su 12 chiusi** | Lavoro tecnico | 🟠 **In corso. 6/09: `margini.py` (1.377 righe) chiuso** — `get_analisi_centri` leggeva lo snapshot mentre il gemello nello stesso file fondeva l'override mensile: **4 sedi** con fatturato sbagliato, una a **0,00 EUR invece di 402.168**. Rimossa l'eccezione nella guardia di dominio (vera per lo split, falsa per il totale netto). 8 mutanti, 7 uccisi. **Prossimo: `fatture.py`** (4/15 endpoint nei test, 4,05 M€, alimentato ieri). ⚠️ La tabella «testati» del prompt era **sovrastimata**: ri-contando i path negli test, `fatture.py` e' 4/15 e non 8/15. `admin.py` resta **ultimo** (solo staff). Residui dichiarati nel verbale: snapshot `mol` stantio (43/75 righe, **0 lettori**), blocco Streamlit morto in `margine_service.py` (5 funzioni), formula MOL in 7 punti (le 5 Python identiche). **È il candidato vero, non il frontend.** **16.915** righe (ri-misurate il 6/09), ~5.400 lette: il blocco più grande a copertura parziale, ed è **backend, dove passano i soldi**. Il perimetro *sicurezza* è chiuso (216 endpoint su 216), la *logica* no. Ri-misurati il 06/09: `admin.py` **3.200** righe (il più grande, fra i meno presidiati, ma solo staff), `workspace.py` **2.494** (non 2.365), `gruppo.py` 2.385, `ricavi.py` 1.506. **Un router per volta**, scegliendo dopo aver misurato a DB quali tabelle serve. Prompt pronto: `docs/piani/PROMPT_ROUTER_ULTIMA_ZONA.md`. Include il ripasso **#3 categorizzazione** (regola di dominio #1) |
