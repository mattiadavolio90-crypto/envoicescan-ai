...
| 05/09 (notte) | Il terzo consumatore del modello ore extra, e la scrittura che lo permetteva | chiusa — **18/18 mutanti** (4 sopravvissuti alla 1ª stesura, tutti test deboli); 4 passate di code-reviewer, verde alla 4ª; l'aggregazione desktop gonfiava monte ore e costo del 25%; regola, aggregazione e costo turno in una fonte unica (`lib/ore-turno.ts`) provata al **punto d'uso**; dato impossibile rifiutato in scrittura su tutti e 4 gli endpoint. Code-reviewer: 2 blocchi (addendo in colonna sbagliata, PATCH mensile senza guardia sui negativi) + il mutante al call-site sopravvissuto |
...
| 15/09 | **Lente trasversale L7 — fatture ostili in ingresso** | chiusa — FatturaPA avversarie **eseguite** attraverso il parser vero. **3 difetti corretti**: gli importi non finiti (`nan`/`inf`) passavano **entrambe** le copie di `_to_float_safe` e ogni ramo di `_to_int_safe`, che in piu' esplodeva con `OverflowError` fuori dal suo `except`; e una **scrittura incompleta taciuta** (ritorno `righe: 0` con 500 righe gia' a DB; tetto di 2.000 righe che tronca e si auto-certifica "OK"). La verifica avversaria ha smentito **due volte** me stesso: il primo fix era **parziale** (il percorso Vision convertiva con `float()` nudi fuori da ogni helper) e una mia frase era **falsa** (il NaN non arriva a DB: e' `httpx` con `allow_nan=False` a fermarlo, non il nostro codice). Il terzo l'ha trovato il controllo delle **inerenze**: `/api/upload/invoice` ripeteva lo stesso `righe_salvate=0` un piano piu' su, con un consumatore frontend vero. 28/28 mutanti. Il code-reviewer (verde) ha trovato **due presidi che non coprivano quello che dicevano** — il guardiano dell'equivalenza si skippava in CI per il checkout shallow, e le righe del worker che segnalano il troncamento non erano provate da nulla: chiusi entrambi, non rinviati. Misurato prima di allarmarsi: 0 righe non finite su 39.649, e su 3.558 documenti il piu' lungo ha 657 righe — il tetto non l'ha mai toccato nessuno |
...
che cambia, non un refactor neutro.

### La quarta passata: la stessa divergenza, sul lato che non avevo guardato

Il verdetto e' stato **verde**, ma il reviewer ha aggiunto una nota che valeva
un blocco: quella scelta l'avevo applicata al frontend e **non al backend**.
`workspace.py:1284` (`co_ext_eff`, che alimenta la pagina Personale e l'export
Excel) **pagava comunque le extra** con la sola tariffa maggiorata, mentre
`costoTurnoGiornaliero` ritornava 0. **Lo stesso turno, due importi diversi a
seconda di dove lo si guardava** — letteralmente la classe di difetto che questa
sessione esiste per chiudere, ricomparsa sull'asse che avevo appena creato
correggendo il frontend.

Verificato che `margini.py` era gia' corretto (`costo_orario` NULL → `continue`,
riga contata in `n_senza_costo`): **il MOL non e' mai stato toccato**, divergeva
solo l'export. Backend allineato al frontend, mutante 18 ucciso.

La lezione non e' "il reviewer trova cose": e' che **cambiare il comportamento in
un punto crea una divergenza nei punti che non cambi**. Correggere il frontend
ha reso sbagliato un backend che prima era coerente. Ogni volta che si sceglie
cosa fare di un dato mancante, la scelta va portata a **tutti** i percorsi che
leggono quel dato, nello stesso commit.

**Impatto oggi: zero righe** (`costo_orario` NULL su 107/107, nessuna tariffa
extra).

**Bilancio mutazione: 18 mutanti, 18 uccisi**, dopo che **4 sono sopravvissuti
