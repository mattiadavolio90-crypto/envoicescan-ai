...
def carica_margini_anno(user_id: str, ristorante_id: str, anno: int) -> dict:
    ...
    try:
        ...
        response = supabase.table('margini_mensili') \
            .select('mese, fatturato_iva10, fatturato_iva22, altri_ricavi_noiva, altri_costi_fb, altri_costi_spese, quote_riparto_fb, quote_riparto_spese, costo_dipendenti, costo_personale_extra, mol') \
            .eq('user_id', user_id) \
            .eq('ristorante_id', ristorante_id) \
            .eq('anno', anno) \
            .execute()
