...
| Layer | Percorso | Note |
|---|---|---|
| Frontend (produzione) | `apps/web/` | Next.js 16 (App Router) su Vercel — 14 aree app + auth/legal/mobile, 173 route API (`find apps/web/src/app/api -name route.ts`, 25/09/2026) |
...
| Worker API | `services/fastapi_worker.py` (9.979 righe) | FastAPI — `/health`, `/api/*`; logica nei router `services/routers/*.py` |
...
| Migrations | `supabase/migrations/*.sql` (canonico, 150 file tracciati — `git ls-files`, 24/09/2026) | Schema PostgreSQL, RLS, trigger. `migrations/*.sql` è LEGACY storico, 91 file su numerazione `001`–`082` (vedi `migrations/_LEGGIMI_STATO.md`) |
| Test | `tests/*.py` | **14.184 verdi + 45 skip** al commit `05640af` del 17/09/2026 — cifra **al netto** dei 539 `-m sql`, cioe' da `-m "not sql"`; il run di default (`python -m pytest`) li **include** e il 18/09 sera, al commit `47657a2`, dava **15.485 + 45 skip = 15.530 raccolti** (+722 dalla mattina: i 41 del contrasto CSS e i 674 del presidio «colori solo token» della fase 2). La cifra `-m "not sql"` qui sopra e' quella **misurata il 17/09**: si ri-misura, non si ottiene per sottrazione. Dichiara sempre con quale comando hai misurato — il totale **si muove coi file di CODICE**: **+8 per ogni `.tsx` nuovo** sotto `apps/web/src`, da TRE presidi che camminano sul filesystem (`test_colori_solo_token` +5, `test_niente_nomi_file_a_video` +2, `test_troncamenti_recuperabili` +1 — misurato il 24/09/2026 creando un file di prova; il +5 del solo primo presidio non e' il totale), e da `tests/` sono 9 in meno che dalla root. **Non** coi file `.md`: `test_documentazione_onesta` è parametrizzato su `DOC_VIVI`, una lista fissa — verificato il 24/09/2026 creando un `.md` nuovo, 61 test prima e dopo. **545 su un Postgres vero** (`-m sql`, misurati il 24/09/2026 con `python -m pytest tests/ -m sql -q -p no:randomly`; di cui 315 dell'isolamento fra clienti) + **126 Deno** (misurati il 25/09/2026 con `deno test --allow-env --allow-net supabase/functions/**/*_test.ts`, il comando della CI: sulla *directory* invece che sul glob si include `test.ts`, che non e' un test e chiama `Deno.exit(1)`, e si ottiene un falso rosso). Frontend: nessun runner npm — vedi Trappole |
...
- **Né `tsc` né un test verde provano che il codice funzioni.** `tsc --noEmit` non
  ...
  sul **testo del sorgente**. Le funzioni SQL vanno **eseguite** (`-m sql`, 545): lì
