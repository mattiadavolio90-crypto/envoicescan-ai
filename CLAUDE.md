...
| Test | `tests/*.py` | **14.620 verdi + 45 skip** il 15/09/2026 — il totale **si muove coi file `.md`** (`test_documentazione_onesta` è parametrizzato su quelli), e da `tests/` sono 9 in meno che dalla root. **539 su un Postgres vero** (`-m sql`, di cui 315 dell'isolamento fra clienti) + 101 Deno. Frontend: nessun runner npm — vedi Trappole |
...
- **Né `tsc` né un test verde provano che il codice funzioni.** `tsc --noEmit` non
  ...
  sul **testo del sorgente**. Le funzioni SQL vanno **eseguite** (`-m sql`, 538): lì
