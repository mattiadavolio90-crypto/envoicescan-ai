...
# Le sequenze di una colonna IDENTITY (pg_depend deptype 'i') le crea la colonna
# stessa: emetterle qui prima farebbe fallire il CREATE TABLE ("already exists").
Q_SEQUENZE = """
SELECT c.relname
...
WHERE c.relkind = 'S' AND n.nspname = 'public'
  AND NOT EXISTS (
      ...
      WHERE dep.objid = c.oid AND dep.classid = 'pg_class'::regclass AND dep.deptype = 'i')
ORDER BY c.relname;
"""
...
# `attidentity` marca le colonne IDENTITY: non hanno una riga in pg_attrdef,
# quindi senza questo ramo uscivano come `id bigint NOT NULL` senza default —
# gruppo_tags e gruppo_tag_prodotti (debito dichiarato da L2 il 14/09/2026,
# rattoppato nella fixture di tests/test_isolamento_per_risorsa.py fino al 15/09).
Q_TABELLE = """
