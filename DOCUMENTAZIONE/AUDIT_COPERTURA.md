...
| **L5 — Ciclo di vita di colonne e campi** | 15/09/2026 | `scripts/audit_ciclo_vita_colonne.py`: una lettura di 540 file di codice + 10 Edge Function + 149 SQL, letture e scritture separate per ognuna delle 667 colonne (AST Python per il builder Supabase, regex per TypeScript e SQL, trigger e default inclusi), incrociate con la fotografia dei dati live (righe / non-NULL / distinti per colonna, `L5_stat_colonne_2026-09-15.csv`); taratura sui casi di esito noto; le 47 colonne NULL al 100% lette una per una al call site; 11 mutanti sul rilevatore + 1 sullo snapshot, 12 uccisi | `docs/storico/audit-2026-09/CICLO_DI_VITA_COLONNE.md` + `tests/test_audit_ciclo_vita_colonne.py` (13 presidi). **Nessuna colonna letta-e-mai-scritta che produca un numero sbagliato in pagina**: 4 lo sono per costruzione (`fattore_kg` ×2, che il frontend manda sempre null; `users.session_token` ×2, ramo legacy senza piu' dati), le altre 43 NULL al 100% hanno uno scrittore vero e nessun cliente le ha mai compilate. 11 colonne morte + 2 tabelle morte proposte per il drop, 4 decisioni per Mattia (il code-reviewer ha bocciato 2 tabelle «morte» del primo giro: `brand_ambigui` viva in `ai_service.py`, `email_rate_log` presupposta da un job GDPR). Saldato il debito di L2: identity di `gruppo_tags` emessa dal generatore dello snapshot, 4 colonne mancanti aggiunte, rattoppo tolto dalla fixture | quando si aggiunge una colonna o si rigenera lo snapshot: `--taratura` e poi l'inventario; il CSV dei dati si rifa' sul DB live (query nel docstring); `category_change_log.actor_*` si rimisura al primo cambio di categoria su `fatture` dopo il deploy dell'8/9 |
| **L6 — Tempo, concorrenza, dipendenze che cadono** | 15/09/2026 | un caso **eseguito** per famiglia, con l'osservabile del cliente: ora congelata alle 00:30 del 1° del mese a Roma (server al giorno prima) attraverso l'endpoint di upload vero; due `claim_batch_for_processing` concorrenti, due `assegna_fattura_a_sede` (thread che aspetta il commit) e il lock del lotto invecchiato sotto un secondo processo, su Postgres vero; `openai.RateLimitError` sollevata sotto il `@retry` di produzione. Perimetro rimisurato con l'AST, non col grep: 6 letture dell'ora senza fuso nel codice (2 dei «8» erano commenti), 11 chiamate HTTP **tutte** con timeout (gli «8 senza» erano chiamate multi-riga), 3 client OpenAI su 4 col default del SDK (600 s, 2 retry nascosti). Verifica avversaria in workflow (2 + 3 agenti, sola lettura) sui rilievi; 11 mutanti, 11 uccisi (2 sullo snapshot) | `tests/test_upload_policy_giorno_di_roma.py` (3), `tests/test_sql_concorrenza_coda.py` (4, `-m sql`), `tests/test_ai_429_attraverso_il_retry_vero.py` (4), `tests/test_openai_client_timeout.py` (3), `tests/test_worker_lock_per_item.py` (6). **3 difetti corretti**: la policy date dell'upload decideva col giorno UTC (`oggi=_oggi_rome()` al call site); i client OpenAI di classificazione senza timeout (`OPENAI_TIMEOUT_SECONDS = 90`, fabbrica `_nuovo_client_openai`, guardia AST su ogni `OpenAI(`); **il lock della coda era del lotto ma il lavoro dell'item** — dopo due timeout il terzo item partiva con un lock gia' stantio e un secondo processo (deploy, drain da GitHub, worker locale) lo riprendeva mentre il primo lo chiudeva: `_rinnova_lock` a ogni item, item saltato se il lock non e' piu' suo (il refutatore ha smentito il mio «irraggiungibile»). Dichiarati: `mark_queue_item_done`/`schedule_retry` restano senza controllo del chiamante, quota chat sul giorno UTC (decisione), SDK 2 retry × tenacity 3, client OpenAI nuovo a ogni classificazione | quando cambia `JOB_TIMEOUT`/`STALE_LOCK_MIN` (il presidio sui default lo dice) o si aggiunge un secondo queue-worker stabile (allora il controllo del chiamante nelle RPC diventa necessario); quando si tocca `_stop_su_tentativi_o_deadline` o le classi ritentabili; quando un nuovo `OpenAI(` o una nuova chiamata HTTP entra nel codice (guardia AST e inventario si rilanciano); al cambio d'ora del 25/10/2026 per i cron delle 06:30 UTC |
...
|---|---|---|---|
| **prossima** | L7 — Fatture ostili in ingresso | ~20 FatturaPA avversarie attraverso parse → guardrail → AI → DB → margini → briefing | Fable `ultrathink` sui casi |
