...
**Righe lette: 24.925 · non lette: 28.035 · totale 52.960** — la tabella copre
...
`lib/` della sera) e di **102 il 2/9** (`piani.ts` 40 + `impostazioni-account.ts` 62), **+58 e poi +32 il 2/9 sera** (crescita di `notifiche-shared.ts`: prima i filtri estratti, poi la CTA mobile). Totale ri-misurato il 2/9 sera con lo stesso metodo (`git archive HEAD` meno i binari): **52.928**, +46 sul mattino — il delta include lavoro di altre sessioni, non solo di questa.
