...
**Righe lette: 24.951 · non lette: 28.037 · totale 52.988** — la tabella copre
...
`lib/` della sera) e di **102 il 2/9** (`piani.ts` 40 + `impostazioni-account.ts` 62), **+58 e poi +38 il 2/9 sera** (crescita di `notifiche-shared.ts`: prima i filtri estratti, poi la CTA mobile). Il totale è **52.988**, misurato il 2/9 sera (a fine sessione) su 413 file tracciati (`git ls-files apps/web/src` meno i binari, sommati con `cat | wc -l`; `wc -l` file per file dà lo stesso numero). **Storia istruttiva di questa singola cifra**: avevo scritto 52.960, il `code-reviewer` ha misurato 52.962 e gli ho dato ragione *spiegando* lo scarto con «file senza newline finale» — una spiegazione **inventata**: in `apps/web/src` non ce n'è nemmeno uno, e nessuno dei due numeri era quello vero. Lo scarto veniva dal set di esclusione (gli `.svg`). **Accettare una correzione non è verificarla, e giustificare a posteriori uno scarto che non si è misurato è il modo tipico di rendere definitiva una cifra sbagliata** — che è precisamente ciò che questo file documenta. Totale ri-misurato il 2/9 sera con lo stesso metodo (`git archive HEAD` meno i binari): **52.928**, +46 sul mattino — il delta include lavoro di altre sessioni, non solo di questa.
