...
1. ~~**Edge Functions** (30/7, riga 2)~~ — **CHIUSA il 4/8/2026**: 13/13 file
   riletti, `code-reviewer` girato (7/7 ora, ha trovato un gap reale). Il caso
   "2 HIGH integrità coda confluiti in Database" **non era davvero chiuso lato
   Edge Function**: trovato e fixato un HIGH nuovo (race fra rete di sicurezza
   e claim del worker). PR #5 → CI verde su tutti e 3 i check → mergiata
   (`18062a7`). Riga 2 riscritta e passata a 🟢.
2. ~~**AI** (5/7, riga 4, agente Fable)~~ — **CHIUSA il 4/8/2026**: propagazione
   globale (Admin → Categorie → Memoria globale) riesaminata a fondo, trovato e
   fixato 1 HIGH (guardrail NOTE E DICITURE mancante) più un bug preesistente
   più grave scoperto dal `code-reviewer` (l'UPDATE su `fatture` falliva sempre,
   silenziosamente, per una colonna inesistente — la propagazione non aveva mai
   scritto nulla in produzione). Il conteggio "2 HIGH + 4 MED + 4 LOW" del 5/7
   resta **non verificabile** con questa passata (perimetro diverso, non
   l'intero conteggio storico). 3 MEDIUM lasciati aperti per scelta esplicita
   (fix solo l'HIGH). PR #6 → CI verde su tutti e 3 i check → mergiata
   (`a6e0f1e`, deploy verificato via `/health`). Il fix del test che sbloccava
   la CI (`5ed6ce0`) era sfuggito alla review preventiva: `code-reviewer`
   girato **dopo** il merge per chiudere il buco — esito verde, test non vacuo
   (ucciso 2/2 per mutazione). Riga 4 riscritta e passata a 🟢.
3. ~~**Qualità/UI** (19/6, riga 6)~~ — **CHIUSA il 4/8/2026**: la più corta in
   assoluto ("filtro mese uniformato, sky-* → primary") era una passata
   cosmetica mai riverificata come dimensione. Conferma: **è davvero il
   rischio sostanziale più basso delle tre** — 1 solo MEDIUM funzionale reale
   (select morto in Admin → Clienti → Sistema, fixato), resto documentato
   come debito di stile/scelte di prodotto legittime, nessun finding gonfiato
   per sembrare produttivo. `code-reviewer` girato sul diff (fix piccolo,
   controllato con la stessa cura di uno grande). Riga 6 riscritta e passata
   a 🟢. **Con questa, il ciclo 2026-07 ha tutte e 3 le dimensioni rimaste
   riverificate con la stessa profondità delle altre 7.**

Ordine consigliato: fatto Edge Functions, fatto AI, ora Qualità/UI. Ognuna
riscrive la propria riga con data/esito reale al posto di quella attuale, non
aggiunge una riga parallela. **Lezione confermata di nuovo (8/8)**: la regola
"code-reviewer trova sempre un difetto reale" ha retto anche qui — un bug
preesistente sulla colonna `classificato_da` che l'audit originale non aveva
mai scoperto, perché falliva silenziosamente da sempre. **Precisazione onesta
sul conteggio**: la 9ª passata (tardiva, sul solo commit `5ed6ce0`) è tornata
**verde senza findings** — quindi "trova sempre un difetto" non è una legge, e
un ritorno pulito su un diff di 5 righe è l'esito atteso. Il valore dimostrato
resta un altro: le passate su codice *nuovo e sostanziale* hanno trovato
qualcosa 8 volte su 8, e questa 9ª ha comunque prodotto 3 note utili — fra cui
la causa radice del falso verde (il fake client accetta colonne inventate).
