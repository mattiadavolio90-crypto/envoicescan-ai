...
## Prossima sessione: remediation Performance (🟡, findings aperti)
...
**Stato al 3/8/2026 (sera, dopo la remediation)**: **9/10 dimensioni 🟢**, **1 🟡**
(Performance). **Il ciclo NON è chiuso**, ma per un motivo diverso da stamattina:
i **HIGH sono tutti chiusi** (audit + remediation nella stessa giornata, dettaglio
nella riga 5). Restano aperti i **MEDIUM frontend/architetturali** e i **gap di
lettura dichiarati** — nessuno dei quali è un difetto attivo sui clienti.

**Cosa è già stato fatto (non rifarlo):** paginazione dei 12 siti realmente
esposti al cap PostgREST via il nuovo `utils/supabase_paging.py`; `make_cache`
che ora cacha davvero su `TTLCache`; RPC `articoli_da_fatture` per Ricette
(2.058→382 ms); cache condivisa sui 5 endpoint di `prezzi.py` con invalidazione
agganciata a quella di FATTURE (4 tab: 21,2→5,3 s). Suite 10.245 verde.
**Tutto questo è in produzione dal 3/8 sera** (PR #2, merge `3215c06`, CI verde,
worker Railway ripartito su quel commit — verificato via `/health`): non è
lavoro locale in attesa, non ricontrollarlo come se fosse aperto.
...
1. **`upload_handler.py`: 1109 statement, 11% coverage, ZERO righe di test.**
   ...
   delle fatture, il buco di copertura più grande del progetto — e in questa
   remediation ci ho toccato due punti (`:517`, `:845`) senza avere una rete
   sotto. Vale come sessione propria, non come coda di un audit.
2. **I MEDIUM residui di Performance**, tutti frontend/architetturali e nessuno
   urgente: 39 route Next senza `AbortSignal.timeout` (il pattern giusto esiste
   già, `workerFetch`), `variazioni-tab.tsx` senza `useMemo` né virtualizzazione
   (**il pattern corretto è già in `articoli-tab.tsx`**: memo + `PAGE_SIZE=100`),
   N+1 nel queue-worker, bundle **NON MISURATA** (`recharts` statico in 5
   componenti, zero `dynamic()` in `apps/web` — serve `npm install` per pesarla).
3. **Cache per-processo vs `WORKER_WEB_CONCURRENCY=4`.** Limite noto e ora più
   rilevante, visto che le cache sono diventate reali: `clear_fatture_cache()`
   invalida **il processo che ha servito la richiesta**, non gli altri 3. Il TTL
   corto (15s) accorcia la finestra, non la elimina. Se un giorno dà fastidio, la
   strada è invalidazione condivisa (colonna `cache_version` già usata da
   `documenti_service`), non abbassare ancora i TTL.
4. **Zone mai lette** (gap dichiarati, non buchi scoperti dopo): `ricavi.py`,
...
**Lezioni operative (le 5 del 2/8 restano valide, più 5 dal 3/8 mattina, 5 dal 3/8 sera e 7 dalla dimensione Test — 4 dalla Fase 1, 3 dalla Fase 2):**
