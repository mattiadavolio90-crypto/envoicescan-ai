                ...
                <div key={d.id} className="rounded-md border border-border">
                  {editId === d.id ? (
                    ...
                    <div className="flex items-center gap-2 px-3 py-2 group">
                      <span className="min-w-0 font-medium text-sm flex-1 truncate" title={d.nome}>{d.nome}</span>
                    ...
                    <div className="space-y-1 mt-1.5">
                      {disattivati.map(d => (
                        <div key={d.id} className="flex items-center gap-2 px-3 py-2 rounded-md border border-dashed border-border">
                          <span className="min-w-0 text-sm flex-1 truncate text-muted-foreground" title={d.nome}>{d.nome}</span>
      ...
      {turni.length > 0 && (
        <div className="space-y-3">
          {/* NIENTE truncate in queste card: qui ogni colonna e' un numero, e
              ...
              font-black cambia lo spessore del tratto, non la cella del glifo).
              ...
              return (
                <div key={n} className={`rounded-lg border ring-1 ${col.ring} overflow-hidden`}>
                  <div className={`w-full flex items-center justify-between px-4 py-3 hover:${col.bg} transition-colors`}>
                    ...
                    >
                      <span className="font-semibold text-sm truncate" title={n}>{n}</span>
