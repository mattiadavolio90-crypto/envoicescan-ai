...
async function fetchData(token: string) {
  ...
  const [fatRes, ricRes, impRes, mapRes, cliRes] = await Promise.all([
    ...
    fetch(`${WORKER_URL}/api/admin/sistema/ricavi-import`, { headers: h, cache: "no-store" }),
  ...
  return {
    ...
    ricaviImport: impRes.ok ? await impRes.json() : { items: [], counts: {} },
...
export default async function FlussoDatiPage() {
  ...
  const { fatture, ricavi, ricaviImport, mappings, clienti } = await fetchData(token);
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-sm text-muted-foreground">
        Stato di arrivo dei dati per ogni cliente: fatture automatiche (Invoicetronic), ricavi dai gestionali e mapping. Da qui vedi gli alert e sblocchi le fatture rimaste in attesa senza query manuali.
      </p>
      <FlussoDatiClient
        fattureIniziali={fatture}
        ricaviIniziali={ricavi}
        ricaviImportIniziali={ricaviImport}
