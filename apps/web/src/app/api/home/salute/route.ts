...
// Proxy GET per il retry client-side (BlockRetry): vedi commento in briefing/route.ts.
export async function GET() {
  ...
  try {
    const res = await fetch(`${WORKER_URL}/api/home/salute`, { method: "GET", headers, cache: "no-store", signal: AbortSignal.timeout(WORKER_TIMEOUT_MS) });
