...
function workerHeaders(token: string, extra?: Record<string, string>): Record<string, string> {
  const h: Record<string, string> = { Authorization: `Bearer ${token}`, ...extra };
  if (WORKER_SECRET_KEY) h["X-Worker-Key"] = WORKER_SECRET_KEY;
  return h;
}

// PATCH /api/fatture/[id] — aggiorna categoria riga
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const res = await fetch(`${WORKER_URL}/api/fatture/${id}/categoria`, {
      method: "PATCH",
